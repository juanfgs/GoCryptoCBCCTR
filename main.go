...
import(
	"github.com/juanfgs/hmwk_crypto/cbc"
	"github.com/juanfgs/hmwk_crypto/ctr"	
	"fmt"
	"encoding/hex"
//	"log"
)
...
func main() {
	for idx,val := range inputs {
		Key, _:=hex.DecodeString(val.key)		
		fmt.Println(Key)
		c := cbc.New([]byte(Key))
		ct, err := hex.DecodeString(val.ct)
		if err != nil {
			panic(err)
		} 

		fmt.Printf("The PT for #%d is %s \n",idx, c.Decrypt([]byte(ct) ) )
	}
	
	for idx,val := range inputsCTR {
		Key, _:=hex.DecodeString(val.key)		
		fmt.Println(Key)
		c := ctr.New([]byte(Key))
		ct, err := hex.DecodeString(val.ct)
